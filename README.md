...
| `CreateObject`       | Create a new ABAP object (program, class, interface, function group, or include), without source. | `object_type` (`program`\|`class`\|`interface`\|`function_group`\|`include`), `object_name`, `package_name`, `description` (optional), `responsible` (optional), `master_language` (optional, default `EN`), `transport_request` (optional), `class_final`/`class_visibility` (optional, class only) | `@tool CreateObject object_type=program object_name=ZMY_PROGRAM package_name=$TMP description="My program"` |
...
`object_type` for `LockObject`/`UnlockObject`/`SaveObjectSource`/`ActivateObject` must be one of: `program`, `class`, `interface`, `include`, `function_group`, `function_module`, `structure`, `table`, `cds_view`, `behavior_definition`, `service_definition`. For `function_module`, `function_group` must also be supplied. `CreateObject` only supports `program`, `class`, `interface`, `function_group`, `include` (DDIC and RAP objects have a substantially different creation schema and are not covered yet).

**Typical workflows:**
- **New object:** `CreateObject` → `LockObject` → `SaveObjectSource` (with the initial source) → `ActivateObject` → `UnlockObject`.
- **Editing an existing object:** `LockObject` → `SaveObjectSource` (using the returned `lockHandle`) → `ActivateObject` → `UnlockObject`.
