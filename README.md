...
| `CheckObject`        | Run an ADT syntax check against an object's source (same check Eclipse ADT runs on save), without activating it. | `object_type`, `object_name`, `function_group` (if applicable), `version` (`active`\|`inactive`, optional, default `inactive`) | `@tool CheckObject object_type=program object_name=ZMY_PROGRAM version=inactive` |
